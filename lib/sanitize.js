...
/* `data-lang` is a code block's language (lib/codeblock.js). Escaped like
   every attribute value, read only by CSS and by normalizeLang, which maps
   anything unknown to 'plain'. */
const EDITOR_DATA_OK = new Set([...DATA_OK, 'data-type', 'data-ds-pagebreak', 'data-lang'])
const EDITOR_ATTRS = {
  ...ATTRS,
...
const CSS_PROPS = new Set([
  ...
  /* A columns row's stored widths (lib/columns.js). Without this every
     columns row lost its widths on the next load and fell back to one
     stacked column. It sizes grid tracks and cannot fetch anything; the
     value check below still applies, and minmax() is the only function it
     brings with it. */
...
/* THE PROPERTY LIST WAS AN ALLOWLIST AND THE VALUE CHECK WAS NOT.
   ...
   network request. */
const CSS_FN_OK = new Set(['rgb', 'rgba', 'hsl', 'hsla', 'var', 'calc', 'minmax'])
