...
export function blocksToHtml(blocks, name, { forPrint = false } = {}) {
  ...
  const body = blocks.map((b, i) => {
    ...
    if (b.type === 'text') {
      // Already HTML from the contentEditable; strip scripts/handlers defensively.
      const clean = String(b.content || '')
...
export function exportPng(blocks, name, { dark = false } = {}) {
  ...
  const ink = dark ? '#E8E6E1' : '#1A1917'
  const mut = dark ? '#9A9790' : '#6B6860'
  const line = dark ? '#2E2D29' : '#D5D1C7'
  const bg = dark ? '#1A1917' : '#FFFFFF'
  const head = dark ? '#262522' : '#F1EFE9'
