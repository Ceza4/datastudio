...
/* ── OVER-RENDERING FOR THE CANVAS'S OWN ZOOM ─────────────────────────────
   The infinite canvas scales the whole block with a CSS transform, and
   NotebookCanvas's wheel handler clamps that transform explicitly:

     const newZ = Math.min(3, Math.max(0.25, oldZ * Math.exp(-e.deltaY * gain)))

   0.25× to 3×, a hard ceiling in the multiplicative formula rather than a soft
   default — there is no way to zoom past 3× anywhere in the app. So a page
   rasterised at 1× CSS looks correct at 1× canvas zoom and progressively
   softer up to 3×, which is exactly the blur that reads as "this tool is low
   quality" on a document viewer.

   BECAUSE THE CEILING IS FIXED AND KNOWN, A STATIC OVER-RENDER IS SUFFICIENT.
   Rendering at 3× the CSS size covers every zoom level the UI allows, and a
   bitmap displayed SMALLER than it was rendered never looks worse — only the
   reverse direction blurs. The usual objection to the static approach ("enough
   zoom and it blurs again anyway") does not apply when the app has the same
   hard ceiling as the buffer.

   Which is also why there is no debounced re-render on zoom here. It would
   work, but it would be solving a problem that does not exist — and it would
   cost a re-render path through lib/renderqueue.js's promise-chain
   serialisation plus the asynchronous-cancel handling that comes with it.
   Revisit only if the zoom cap itself changes. */
export const CANVAS_ZOOM_MAX = 3

/**
 * Device-pixel-ratio-aware canvas size, with an optional over-render factor.
 *
 * `overRender` defaults to 1 so the two-argument behaviour is unchanged; the
 * PDF block passes CANVAS_ZOOM_MAX.
 *
 * THE TWO FACTORS MULTIPLY, AND THE PIXEL CAP IS WHAT ABSORBS IT. dpr and
 * canvas zoom are independent magnifications of the same bitmap, so the honest
 * combined ratio is dpr × overRender — which on a 2× display is 6, i.e. 36× the
 * pixel area of a 1× render. A4 at 6× is past MAX_CANVAS_PIXELS, so the cap
 * below scales it back down and the page ends up as sharp as the browser will
 * allow rather than as sharp as the arithmetic asked for.
 *
 * That is the right failure mode and it is worth being explicit about: the cap
 * degrades sharpness smoothly and preserves aspect ratio, where an uncapped
 * allocation fails outright and paints nothing. So "3× over-render" is the
 * ceiling this aims at, not a guarantee at every page size and display density.
 */
export function canvasSizeFor(viewport, dpr = 1, overRender = 1) {
  const over = Math.min(Math.max(Number.isFinite(overRender) ? overRender : 1, 1), CANVAS_ZOOM_MAX)
  const ratio = Math.min(Math.max(Number.isFinite(dpr) ? dpr : 1, 1), 3) * over
