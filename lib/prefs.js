...
export const DEFAULT_PREFS = {
  ...
  reduceMotion: null,
  /* What a newly-dropped image lands as: a full block, or a collapsed icon.

     For the deliberate icon-first import — a batch of reference shots you want
     as a tidy list rather than a wall of photos. 'full' stays the default, so
     nothing changes for anyone who never opens Settings. Per-image curation
     after the fact is the Image rail's Display button; this is only about where
     a drop STARTS. */
  imageDropMode: 'full',
}

export const GRID_SIZES = [16, 24, 32, 48, 64]
export const IMAGE_DROP_MODES = ['full', 'icon']
...
export function normalizePrefs(raw) {
  ...
  return {
    ...
    /* Anything that is not exactly one of the two known values falls back to
       'full', the same discipline gridSize uses — a prefs object from a newer
       build, a half-finished migration or a hand-edited localStorage entry must
       never be able to produce a mode nothing knows how to render. */
    imageDropMode: IMAGE_DROP_MODES.includes(raw.imageDropMode) ? raw.imageDropMode : DEFAULT_PREFS.imageDropMode,
  }
}
