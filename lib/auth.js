 ...
 *
 * Supabase does not switch the address on this call. It sends a confirmation
 * link, and the change lands when that link is followed. With "secure email
 * change" on (the default), the CURRENT address must confirm too. So the
 * message says the old address still works until then. Saying "Email changed"
 * here would be false, and the person would try to sign in with an address
 * that does not work yet.
 *
 * The redirect is built from the current origin, for the same reason as
 * requestPasswordReset: a link that arrives by email is the last thing that
 * should take its target from a parameter.
 */
export async function changeEmail(nextInput, currentEmail, client) {
  const c = client === undefined ? await getSupabase() : client
  const e = validateEmail(nextInput)
  if (!e.ok) return { status: AUTH.INVALID, message: e.message, field: 'email' }
  if (currentEmail && e.email.toLowerCase() === String(currentEmail).toLowerCase()) {
    return { status: AUTH.INVALID, message: 'That is already your email address.', field: 'email' }
  }
  if (!c) return { status: AUTH.UNCONFIGURED, message: unconfiguredMessage() }

  try {
    const origin = typeof window !== 'undefined' ? window.location.origin : ''
    const { error } = await c.auth.updateUser({ email: e.email }, { emailRedirectTo: `${origin}/app` })
    if (error) return { status: AUTH.REJECTED, message: describeAuthError(error) }
    return {
      status: AUTH.OK,
      message: `Confirmation sent to ${e.email}. Until it is confirmed, sign in with ${currentEmail || 'your current address'}. Check that inbox too.`,
    }
  } catch (err) {
    return { status: offline(err) ? AUTH.OFFLINE : AUTH.REJECTED, message: describeAuthError(err) }
  }
