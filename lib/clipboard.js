...
export function materialise(parsed, { x = 0, y = 0 } = {}) {
  ...
  for (const s of parsed.shapes) shapeIds.set(s.id, rid('shape'))
  const shapes = parsed.shapes.map(s => {
    const next = { ...s, id: shapeIds.get(s.id), x: (s.x ?? 0) + x, y: (s.y ?? 0) + y }
    /* A connector follows the shapes it is attached to only when they came
       along. One pasted without them keeps the ends it was copied with (the
       copy bakes them) instead of snapping back onto the originals. */
    if (s.kind === 'connector') {
      next.from = s.from ? shapeIds.get(s.from) || null : null
      next.to = s.to ? shapeIds.get(s.to) || null : null
    }
    return next
  })
  return { blocks, shapes }
