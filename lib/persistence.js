...
export async function saveState(state) {
  ...
  // Structured clone handles the object graph directly, but Sets would throw
  // on the way in, so normalise first. (Nothing currently stores a Set; this
  // is here so that adding one later fails visibly rather than at 3am.)
  let clean
  try {
    clean = JSON.parse(JSON.stringify(payload, (k, v) => (v instanceof Set ? Array.from(v) : v)))
  } catch (err) {
    return { status: SAVE_FAILED, error: `Workspace could not be serialised: ${err.message}` }
  }
  ...
  try {
    await idbSet(STORE_STATE, STATE_KEY, clean)
    const bytes = roughBytes(clean)
    ...
    const quota = await checkQuota('cloudBytes', bytes)
    return quota.ok
...
export async function loadState() {
  ...
  try {
    await idbSet(STORE_STATE, STATE_KEY, { version: 3, ...legacy, savedAt: Date.now() })
