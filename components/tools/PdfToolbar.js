    ...
    <div
      ...
      {PDF_TOOLS.map(t => (
        <button
          ...
          style={{
            width: '100%', height: 29, padding: '0 9px', fontSize: 12,
      ...
      {/* ── history ── */}
      <div style={{ display: 'flex', gap: 4, marginTop: 5, paddingTop: 6, borderTop: `1px solid ${border}` }}>
        ...
        <button onClick={() => st.redo?.()} disabled={!st.canRedo}
          ...
          <Icon name="draw-undo" size={14} style={{ transform: 'scaleX(-1)' }} />
        </button>
      ...
      <button onClick={() => st.extract?.()}
        ...
        className="ds-tbtn"
        style={{ width: '100%', height: 29, marginTop: 5, paddingLeft: 9, paddingRight: 9, fontSize: 12, justifyContent: 'flex-start' }}>
      ...
      <button onClick={() => st.export?.()} disabled={st.exporting}
        ...
        className="ds-tbtn"
        style={{ width: '100%', height: 29, marginTop: 3, padding: '0 9px', fontSize: 12, justifyContent: 'flex-start' }}>
      ...
      <button onClick={() => st.revert?.()} disabled={!hasEdits}
        ...
        className="ds-tbtn"
        style={{ width: '100%', height: 29, padding: '0 9px', fontSize: 12, justifyContent: 'flex-start', color: hasEdits ? 'var(--ds-red)' : undefined, opacity: hasEdits ? 1 : 0.38, cursor: hasEdits ? 'pointer' : 'not-allowed' }}>
      ...
      {hasEdits && (
        <div style={{
          marginTop: 4, fontSize: 11, lineHeight: 1.45, color: 'var(--ds-text-3)',
      ...
      <div style={{
        ...
        fontSize: 11, lineHeight: 1.5, color: 'var(--ds-text-3)',
        display: 'flex', gap: 6,
      }}>
